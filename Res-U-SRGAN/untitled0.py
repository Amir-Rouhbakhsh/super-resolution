...
def Res_UNet(input_tensor, dropout_rate=0.0, batch_norm=True):
    ...
    # Downsampling layers
    conv_128 = res_conv_block(input_tensor, FILTER_SIZE, FILTER_NUM, dropout_rate, batch_norm)
...
# Define a LinkNet block
def linknet_block(x, num_filters):
    # Encoder (downsampling)
    down = layers.Conv2D(num_filters, (3, 3), strides=2, padding='same')(x)
    down = layers.BatchNormalization()(down)
    down = layers.Activation('relu')(down)

    down = layers.Conv2D(num_filters, (3, 3), padding='same')(down)
    down = layers.BatchNormalization()(down)
    
    # Decoder (upsampling)
    up = layers.Conv2D(num_filters, (1, 1), padding='same')(down)
    up = layers.UpSampling2D(size=(2, 2))(up)
    up = layers.Conv2D(num_filters, (3, 3), padding='same')(up)
    up = layers.BatchNormalization()(up)

    # Skip connection
    skip = layers.Conv2D(num_filters, (1, 1), padding='same')(x)
    out = layers.Add()([up, skip])
    out = layers.Activation('relu')(out)
    
    return out


# SRGAN Generator using LinkNet architecture
def build_linknet_srgan_generator(input_shape=(64, 64, 3), num_filters=(64, 128, 256)):
    """
    Builds the SRGAN generator model using a LinkNet architecture.
    """
    # Input Layer (Low-Resolution image)
    input_lr = layers.Input(shape=input_shape)
    ...
    # LinkNet Blocks (You can stack multiple LinkNet blocks here)
    x = linknet_block(x, num_filters[0])
    x = linknet_block(x, num_filters[1])
    x = linknet_block(x, num_filters[2])
...
# Instantiate the generator model with LinkNet and print summary
generator = build_linknet_srgan_generator(input_shape=(64, 64, 3))
generator.summary()
...
def generate_sample_images(epoch, lr_images, hr_images, num_samples=1):
    ...
    for i in range(num_samples):
        ...
        # Save the figure
        sample_dir = 'H:/GPU/srgan thesis/11-unetwithresnet/generated_samples'
...
def generate_sample_images2(epoch, dataset, num_samples=1):
    """
    Generate and save the same SR images during training for comparison.
    Always selects the first batch of the dataset for consistency.
    """
    # Get a specific batch of images from the dataset (e.g., first batch)
    for lr_images, hr_images in dataset.take(1):  # Always take the first batch
        sr_images = generator(lr_images, training=False)
        ...
        for i in range(num_samples):
            ...
            # Save the figure
            sample_dir = 'H:/GPU/srgan thesis/11-unetwithresnet/generated_samples2'
            if not os.path.exists(sample_dir):
                os.makedirs(sample_dir)
            plt.savefig(f"{sample_dir}/epoch_{epoch}_sample_{i + 1}.png")
            plt.close()
...
log_dir = 'H:/GPU/srgan thesis/11-unetwithresnet/logs'
...
checkpoint_dir = 'H:/GPU/srgan thesis/11-unetwithresnet/training_checkpoints'
...
csv_path = 'H:/GPU/srgan thesis/11-unetwithresnet/training_logs4x.csv'
...
def train(dataset, epochs, vgg):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            generate_sample_images2(epoch+1,dataset)
...
save_path = 'H:/GPU/srgan thesis/11-unetwithresnet/generator_model.h5'
generator.save(save_path)
print(f"Final generator model saved to {save_path}")
discriminator.save('H:/GPU/srgan thesis/11-unetwithresnet/discriminator_model.h5')
