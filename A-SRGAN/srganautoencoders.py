...
def autoencoder_block(x, num_filters):
    ...
    This function handles both the encoder and decoder within the same function.
    """
    skip = x
    # Encoder (Downsampling)
    e1 = layers.Conv2D(num_filters[0], (3, 3), padding='same', activation='relu', 
                       activity_regularizer=regularizers.l1(10e-10))(x)
    e2 = layers.Conv2D(num_filters[0], (3, 3), padding='same', activation='relu', 
                       activity_regularizer=regularizers.l1(10e-10))(e1)
    e3 = layers.MaxPooling2D(padding='same')(e2)  # Downsample
    
    # Encoder (more downsampling with increasing filters)
    e4 = layers.Conv2D(num_filters[1], (3, 3), padding='same', activation='relu', 
                       activity_regularizer=regularizers.l1(10e-10))(e3)
    e5 = layers.Conv2D(num_filters[1], (3, 3), padding='same', activation='relu', 
                       activity_regularizer=regularizers.l1(10e-10))(e4)
    e6 = layers.MaxPooling2D(padding='same')(e5)  # Downsample (further)
    
    # Bottleneck layer (most abstract features with 256 filters)
    b = layers.Conv2D(num_filters[2], (3, 3), padding='same', activation='relu', 
                      activity_regularizer=regularizers.l1(10e-10))(e6)

    # Decoder (Upsampling)
    d1 = layers.UpSampling2D()(b)  # Upsample (back to previous resolution)
    d2 = layers.Conv2D(num_filters[1], (3, 3), padding='same', activation='relu', 
                       activity_regularizer=regularizers.l1(10e-10))(d1)
    d3 = layers.Conv2D(num_filters[1], (3, 3), padding='same', activation='relu', 
                       activity_regularizer=regularizers.l1(10e-10))(d2)
    ...
    d5 = layers.Conv2D(num_filters[0], (3, 3), padding='same', activation='relu', 
                       activity_regularizer=regularizers.l1(10e-10))(d4)
    d6 = layers.Conv2D(num_filters[0], (3, 3), padding='same', activation='relu', 
                       activity_regularizer=regularizers.l1(10e-10))(d5)
...
def generate_sample_images(epoch, lr_images, hr_images, num_samples=1):
    ...
    for i in range(num_samples):
        ...
        # Save the figure
        sample_dir = 'H:/GPU/srgan thesis/6-srganautoencoders/generated_samples'
...
log_dir = 'H:/GPU/srgan thesis/6-srganautoencoders/logs'
...
checkpoint_dir = 'H:/GPU/srgan thesis/6-srganautoencoders/training_checkpoints'
...
csv_path = 'H:/GPU/srgan thesis/6-srganautoencoders/training_logs4x.csv'
...
end_time = time.time()
save_path = 'H:/GPU/srgan thesis/6-srganautoencoders/generator_model.h5'
generator.save(save_path)
print(f"Final generator model saved to {save_path}")
discriminator.save('H:/GPU/srgan thesis/6-srganautoencoders/discriminator_model.h5')
...
generator = tf.keras.models.load_model('H:/GPU/srgan thesis/6-srganautoencoders/generator_model.h5')
...
output_dir = 'H:/GPU/srgan thesis/6-srganautoencoders/generated_hr_images'
