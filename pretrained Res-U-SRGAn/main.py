...
'''
# Optionally, you can check the output shape of unet_cut
dummy_input = tf.random.normal([1, 64, 64, 64])  # Random input to check shape (batch size 1)
output = unet_cut(dummy_input)
'''
...
def generate_sample_images2(epoch, dataset, num_samples=1):
    ...
    for lr_images, hr_images in dataset.take(1):  # Always take the first batch
        ...
        for i in range(num_samples):
            ...
            # Save the figure
            sample_dir = 'H:/GPU/srgan thesis/13-pretrainedunet/generated_samples2'
            if not os.path.exists(sample_dir):
                os.makedirs(sample_dir)
            plt.savefig(f"{sample_dir}/epoch_{epoch}_sample_{i + 1}.png")
            plt.close()
