...
def build_generator(num_res_blocks=16, input_shape=(None, None, 3)):
    """
    Builds the SRGAN generator model.
    """
    def residual_block(x):
        ...
        # Use PReLU with shared axes for spatial dimensions
    ...
    # Input Layer (Low-Resolution image) with explicit input shape
    input_lr = layers.Input(shape=input_shape)  # Define input shape explicitly
    
    # Initial Convolution + PReLU
    x = layers.Conv2D(64, (9, 9), padding='same')(input_lr)
    ...
    x = layers.PReLU(shared_axes=[1, 2])(x)  # PReLU with shared axes for 2D inputs
    x = layers.Lambda(lambda x: tf.nn.depth_to_space(x, 2))(x)  # Pixel Shuffle (2x upsampling)

    x = layers.Conv2D(256, (3, 3), padding='same')(x)
    x = layers.PReLU(shared_axes=[1, 2])(x)  # PReLU with shared axes for the upsampled feature map
...
# Instantiate the generator model and print summary
generator = build_generator(input_shape=(32, 32, 3))  # Set input shape explicitly
generator.summary()
...
def generate_sample_images(epoch, lr_images, hr_images, num_samples=1):
    ...
    for i in range(num_samples):
        ...
        # Save the figure
        sample_dir = 'H:/GPU/srgan thesis/8-autoencoder resnet/generated_samples'
...
log_dir = 'H:/GPU/srgan thesis/5-Srgan tensorflow/logs'
...
checkpoint_dir = 'H:/GPU/srgan thesis/5-Srgan tensorflow/training_checkpoints'
...
csv_path = 'H:/GPU/srgan thesis/5-Srgan tensorflow/training_logs4x.csv'
...
save_path = 'H:/GPU/srgan thesis/5-Srgan tensorflow/generator_model.h5'
generator.save(save_path)
print(f"Final generator model saved to {save_path}")
discriminator.save('H:/GPU/srgan thesis/5-Srgan tensorflow/discriminator_model.h5')
...
generator = tf.keras.models.load_model('H:/GPU/srgan thesis/5-Srgan tensorflow/generator_model.h5')
...
output_dir = 'H:/GPU/srgan thesis/5-Srgan tensorflow/generated_hr_images'
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
'''
def save_lr_images(dataset, output_dir):
    """
    This function saves the low-resolution images from the dataset to a specified directory.
    """
    ...
    # Iterate over the dataset
    for i, (lr_images, hr_images) in enumerate(dataset):
        # Convert and save each LR image
        for j in range(lr_images.shape[0]):
            lr_image = lr_images[j]  # Extract the low-resolution image
            
            # Rescale the LR image to [0, 255] for saving
            lr_image = (lr_image + 1.0) / 2.0  # Rescale to [0, 1]
            lr_image = np.clip(lr_image, 0, 1)  # Ensure pixel values are within [0, 1]
            lr_image = (lr_image * 255).astype(np.uint8)  # Convert to [0, 255]

            # Convert the NumPy array to an image
            img = array_to_img(lr_image)
            
            # Save the image to the output directory
            img.save(os.path.join(output_dir, f'lr_image_{i}_{j}.png'))

        print(f"Processed and saved batch {i + 1} LR images")
        
# Specify the output directory where LR images will be saved
lr_output_dir = 'H:/GPU/srgan thesis/5-Srgan tensorflow/high_resolution_images'

# Call the function to save LR images
save_lr_images(dataset, lr_output_dir)
